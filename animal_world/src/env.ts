        ...
        "殴打水草": {
            ...
            solve: (ctx: seal.MsgContext, msg: seal.Message, players: Player[]) => {
                ...
                if (Math.random() < 0.5) {
                    ...
                    seal.replyToSender(ctx, msg, `<${player.name}>被水草缠住了，HP-1`);
                    return;
                }
                ...
                seal.replyToSender(ctx, msg, `<${player.name}>战胜了水草，新的词条：${entry[0].name}`);
            }
        },
        "殴打乌龟": {
            name: "殴打乌龟",
            ...
            solve: (ctx: seal.MsgContext, msg: seal.Message, players: Player[]) => {
                ...
                let text = `<${fish.name}>殴打了<${turtle.name}>，${crit ? `暴击了，` : ``}打掉了${damage}血`
                ...
                if (turtle.animal.attr.hp <= 0) {
                    ...
                    seal.replyToSender(ctx, msg, text + `\n<${fish.name}>打爆了<${turtle.name}>，新的词条：${entry[0].name}`);
                    turtle.revive();
                    return;
                }
