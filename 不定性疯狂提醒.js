...
if (!ext) {
    ext = seal.ext.new('crazycheck', '错误', '1.0.1');
    // 注册扩展
    ...
    cmdsc.solve = (ctx, msg, cmdArgs) => {
        if (ctx.group.logOn) setTimeout(() => {if (checksan(ctx)) seal.replyToSender(ctx, msg, seal.ext.getStringConfig(ext, '不定性疯狂提醒').replace('{{调查员}}', ctx.player.name));}, 500)
    };
    ...
    cmdst.solve = (ctx, msg, cmdArgs) => {
        ...
        if (/san/.test(val)) if (ctx.group.logOn) setTimeout(() => {if (checksan(ctx)) seal.replyToSender(ctx, msg, seal.ext.getStringConfig(ext, '不定性疯狂提醒').replace('{{调查员}}', ctx.player.name));}, 500)
    };
