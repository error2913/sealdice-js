...
if (!ext) {
    ...
    seal.ext.registerTemplateConfig(ext, '展示队伍', ['队伍如下：\n{{队伍列表}}'])
    ...
    seal.ext.registerTemplateConfig(ext, '抽取成员', ['抽到了：\n{{成员名称}}'])

    seal.ext.registerTemplateConfig(ext, '呼叫成员', ['下面的人来跑团啦！请在限定时间{{时间限制}}s内回复“到”：\n{{成员列表}}'])
    seal.ext.registerTemplateConfig(ext, '呼叫结束', ['应到{{当前人数}}人，实到{{签到人数}}人，未到{{咕咕人数}}人。未到如下：\n{{成员列表}}'])
    seal.ext.registerIntConfig(ext, '呼叫时间限制（s）', 60)

    seal.ext.registerTemplateConfig(ext, '展示属性', ['属性如下：\n{{展示属性列表}}'])
    seal.ext.registerTemplateConfig(ext, '修改属性', ['修改如下：\n{{修改操作}}\n{{修改属性列表}}'])
    seal.ext.registerTemplateConfig(ext, '排序属性', ['排序如下：\n{{排序属性列表}}'])
    ...
    class Team {
        ...
        bindTeam(groupId, name) {
            ...
            if (index !== -1) {
                ...
                //删除默认队伍
                let defaultIndex = teams.findIndex(item => item.name === '默认队伍');
                if (defaultIndex!== -1 && teams[defaultIndex].members.length === 0) {
                    teams.splice(defaultIndex, 1);
                }
        ...
        showTeamList(groupId) {
            ...
            let teamsText = teams.map(item => {
                if (item.name == team.name) return `${item.name} ${item.members.length}人◎`;
                else return `${item.name} ${item.members.length}人`;
            })

            const tmpl = seal.ext.getTemplateConfig(ext, "展示队伍")
            return tmpl[Math.floor(Math.random() * tmpl.length)]
                .replace('{{队伍列表}}', teamsText.join('\n'))
        }
        ...
        drawMember(groupId, num, epId, guildId) {
            const team = this.getBindData(groupId)
            ...
            let result = []
            let copy = team.members.slice();

            while (result.length < num && copy.length > 0) {
                let randomIndex = Math.floor(Math.random() * copy.length);
                const userId = copy.splice(randomIndex, 1)[0]
        ...
        callMembers(groupId, epId, guildId) {
            ...
            setTimeout(() => {
                ...
                let membersText = call.map(userId => {
        ...
        showAttrb(groupId, attrb, epId, guildId) {
            ...
            let membersText = team.members.map(userId => {
                ...
                if(!attrb)return `${mctx.player.name} hp${seal.vars.intGet(mctx, 'hp')[0]}/${Math.floor((seal.vars.intGet(mctx, 'con')[0] + seal.vars.intGet(mctx, 'siz')[0]) / 10)} san${seal.vars.intGet(mctx, 'san')[0]}/${seal.vars.intGet(mctx, 'pow')[0]} dex${seal.vars.intGet(mctx, 'dex')[0]}`;
                else return `${mctx.player.name} ${attrb}${seal.vars.intGet(mctx, attrb)[0]}`
            });

            const tmpl = seal.ext.getTemplateConfig(ext, "展示属性");
            return tmpl[Math.floor(Math.random() * tmpl.length)]
                .replace('{{展示属性列表}}', membersText.join('\n'));
        }
        ...
        setAttrb(groupId, val, epId, guildId) {
            ...
            let membersText = team.members.map(userId => {
                let mctx = getCtxById(epId, groupId, guildId, userId);
                return `${mctx.player.name} ${seal.vars.intGet(mctx, attrb)[0]}=>${seal.format(mctx, result)}`;
            });

            const tmpl = seal.ext.getTemplateConfig(ext, "修改属性");
            return tmpl[Math.floor(Math.random() * tmpl.length)]
                .replace('{{修改操作}}', opText)
                .replace('{{修改属性列表}}', membersText.join('\n'))
        }
        ...
        sortAttrb(groupId, attrb, epId, guildId) {
            ...
            let membersText = ctxlst.map(ctx => {
                return `${ctx.player.name} ${attrb}${seal.vars.intGet(ctx, attrb)[0]}`;
            });

            const tmpl = seal.ext.getTemplateConfig(ext, "排序属性");
            return tmpl[Math.floor(Math.random() * tmpl.length)]
                .replace('{{排序属性列表}}', membersText.join('\n'));
        }
    }
...
【.team sort 属性名】对成员的该项属性排序`;
    ...
    cmdteam.solve = (ctx, msg, cmdArgs) => {
        ...
        switch (val) {
            ...
            case 'draw': {
                seal.replyToSender(ctx, msg, team.drawMember(groupId, val2, ctx.endPoint.userId, msg.guildId))
                return seal.ext.newCmdExecuteResult(true);
            }
            case 'call': {
