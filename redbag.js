...
if (!ext) {
    ext = seal.ext.new('redbag', '错误', '1.1.0');
...
cmdSend.solve = (ctx, msg, cmdArgs) => {
    ...
    if (uid !== muid) {
        ...
        seal.replyToSender(ctx, msg, `[CQ:image,file=${url}/send_exclusive_redbag?user_id=${uid.replace(/\D+/g, '')}&user_name=${un.replace(/\]/g, '%5D').replace(/,/g, '%2C')}&target_user_id=${muid.replace(/\D+/g, '')}&target_user_name=${mun.replace(/\]/g, '%5D').replace(/,/g, '%2C')}&amount=${amount}&text=${text ? text.replace(/\]/g, '%5D').replace(/,/g, '%2C') : '恭喜发财，大吉大利'}]`);
    } else {
        ...
        seal.replyToSender(ctx, msg, `[CQ:image,file=${url}/send_redbag?user_id=${uid.replace(/\D+/g, '')}&user_name=${un.replace(/\]/g, '%5D').replace(/,/g, '%2C')}&amount=${amount}&total=${total}&text=${text ? text.replace(/\]/g, '%5D').replace(/,/g, '%2C') : '恭喜发财，大吉大利'}]`);
    }
...
cmdGrab.solve = (ctx, msg, cmdArgs) => {
    ...
    switch (arg) {
        ...
        default: {
            ...
            seal.replyToSender(ctx, msg, `[CQ:image,file=${url}/open_redbag?user_id=${uid.replace(/\D+/g, '')}&user_name=${un.replace(/\]/g, '%5D').replace(/,/g, '%2C')}&sender_user_name=${data[gid][index].un.replace(/\]/g, '%5D').replace(/,/g, '%2C')}&amount=${increase}&total=${total}&remaining=${remaining}&history=${JSON.stringify(history).replace(/\]/g, '%5D').replace(/,/g, '%2C')}]`);
...
cmdShow.solve = (ctx, msg, cmdArgs) => {
    ...
    if (!isNaN(id)) {
        ...
        if (history.length > 0) {
            ...
            s += `[CQ:image,file=${url}/history?total=${total}&remaining=${remaining}&history=${JSON.stringify(history).replace(/\]/g, '%5D').replace(/,/g, '%2C')}]`;
        } else {
            s += `\n数量:${total}`;
        }
